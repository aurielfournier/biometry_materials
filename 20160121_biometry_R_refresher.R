...
# by - Matt Boone (@birderboone on Twitter - 2015) & Auriel Fournier (@RallidaeRule on Twitter - 2015)
...
# data types and how R trys to convert inputs because vectors must be of the same class, 
# R will attempt to convert all inputs to the one class they have in commmon
...
# we've already talked about vectors so we won't cover it here
...
data[50]   # refers to the 50th entry, which is irrespective of the row. 
           # This will not work in a dataframe
...
# list a collection of various data structures. It's like a tacklebox. 
# You can carry the tacklebox together, but its compartmentalized. 
...
# The majority of work is done with data frames and lists. 
# Lists are useful because they're not stored in RAM the same way data.frames are. 
# Dataframes are stored as one big memory piece and everytime you need to append it, it must allocate RAM for the large block, which can eventually lead to whats called a memory leak.
# This is one of the reasons we use lists. 
# Think of lists as giant storage boxes. You can pull from lists as needed and do whats necessary. 
# They're endlessly deep though, so you can easily get lost if you start building lists inside of lists. 
# They're great for storing multiple files, and keeping things organized
...
# symbols
# '-' symbol can subtract in many different ways
5-3
c(3,4,5)
-c(3,4,5)
...
# one of the big tasks in R is subsetting. 
# Once you can subset properly you can slice your data either for analysis or processing and transforming data
...
# loading in the data from the gapminder package
...
# The syntax to base subset works very similar to this. It's basically set on the premises of feeding the dataframe a string of T, F where T is a variable it wants to keep, and F is one its going to ignore. 
# But this process can be complicated to imagine
...
# you can do this and subset which columns you end up with as well. so you can subset by continent and drop the continent column at the same time

test <- datag[datag$continent=='Europe' & datag$year==1987,c('country','lifeExp','gdpPercap')]

# dplyr attempts to clarify complicated statements by using more logical function names and something called pipes. Dplyr is written by a guy named Hadley Whickham, the author of Rstudio. 
# Hadley Whickham has written a suite of useful packages all with the goal of making R easier and more accesible, these include ggplot, dplyr, tidyr.
# Dplyr is a program for transforming data, and the particular function for subsetting is this process called pipes. Basically you feed it a logical set of instructions to do. %>% should be read as 'then'. Dplyr comes with a bunch of new functions, and you can dig into the tutorials on our resource page to learn more on how each function works
...
# I like this idea, and it has many strong functions, it's gaining popularity and we'll try to touch on various dplyr ways of doing thing.
# now dplyr represents a sort of left turn in R, and can be a bit like speaking two languages if you're trying to ride the line of using base R and live in Hadley Wickhams world. We're going to continue on trying to teach both methods dplyr and base R. In some ways dplyr is faster, and in other base R is easier to do complex things.
...
data <- read.csv('./Biometry_Materials/rail_data.csv')
data
...
# in this example we can take our data frame data1, then group by what ever groups we want, and then use either summarise() which will do a by summary on the data frame, or summarise_each which will run a function down the columns. And we can fee it what columns we want the function run on
...
# an apply function is a prewrapped function that loops across data sets. Which makes no sense of course.
